...
"border-bottom":g+"px solid transparent","vertical-align":"middle"};var l={display:"inline-block",width:h/3,height:Math.floor(g)-Math.floor(g)%2,background:b.cssFgColor,"vertical-align":"middle"};var k={display:"inline-block",width:g/3,height:.4*h,background:b.cssFgColor,"vertical-align":"middle"};var q=p.createElement("img");q.alt=b.altAttr;q.style.width="100%";q.style.height="auto";q.style.borderRadius=b.cssRadius;var t=q.cloneNode(!1);q.className="firstSlide";t.style.position="absolute";t.style.top=
0;t.style.left=0;t.style.visibility="hidden";t.style.opacity=0;var c=p.createElement("span");c.className="countSlide";c.style.display="none";c.style.position="absolute";c.style.bottom=0;c.style.left="25%";c.style.right="25%";c.style.backgroundColor="transparent";c.style.cursor="default";c.innerHTML="<span><span>1</span><b>/</b></span>";var m=c.firstChild;m.style.display="inline-block";m.style.padding="0 "+b.cssPadding;m.style.lineHeight=b.cssLineHeight;m.style.fontSize=b.cssFontSize;m.style.fontFamily=
b.cssFontFamily;m.style.fontWeight="bold";m.style.borderRadius=b.cssRadius+" "+b.cssRadius+" 0 0";m.style.opacity=b.cssOpacity;m.style.backgroundColor=b.cssBgColor;n={$skipF:d("<span/>",{"class":"controlSlide hideLastSlide skipFSlide",css:d.extend({},a,{right:b.cssSeparation,bottom:b.cssSeparation}),html:d("<span/>",{css:d.extend({},e,{"border-left":l.width+"px solid "+b.cssFgColor})}).add(d("<span/>",{css:k})).add(p.createTextNode("."))}),$skipB:d("<span/>",{"class":"controlSlide hideFirstSlide skipBSlide",
css:d.extend({},a,{left:b.cssSeparation,bottom:b.cssSeparation}),html:d("<span/>",{css:k}).add(d("<span/>",{css:d.extend({},e,{"border-right":l.width+"px solid "+b.cssFgColor})})).add(p.createTextNode("."))}),$next:d("<span/>",{"class":"controlSlide hideLastSlide nextSlide",css:d.extend({},a,{display:"none",right:b.cssSeparation,top:"50%","margin-top":"-"+h/2+"px"}),html:d("<span/>",{css:l}).add(d("<span/>",{css:d.extend({},e,{"border-left":l.width+"px solid "+b.cssFgColor})})).add(p.createTextNode("."))}),
$prev:d("<span/>",{"class":"controlSlide hideFirstSlide prevSlide",css:d.extend({},a,{left:b.cssSeparation,top:"50%","margin-top":"-"+h/2+"px"}),html:d("<span/>",{css:d.extend({},e,{"border-right":l.width+"px solid "+b.cssFgColor})}).add(d("<span/>",{css:l})).add(p.createTextNode("."))}),$link:d("<a/>",{"class":"controlSlide linkSlide",target:"_blank",css:d.extend({},a,{right:b.cssSeparation,top:b.cssSeparation,"border-radius":b.cssRadius}),html:d("<img/>",{width:"auto",height:2*g,css:{"vertical-align":"middle"}}).add(p.createTextNode("."))}),
...
a.setAttribute("src",a.getAttribute("data-src"));this.fadeInSlide(a,!1)},scaffolding:function(a){var h,g;var e=p.createDocumentFragment();var l=!1;var k=a.slide_image_baseurl_suffix+"?cb="+a.version_no;var q=a.slide_image_baseurl_suffix.split(/[-.]/)[1];var t=Math.round(q*this.settings.aspectRatio);this.toLoad=h=a.total_slides;var c=this;var m=!1;this.loading(!0);this.currentSlide=n.first.cloneNode(!1);this.currentSlide.width=q;this.currentSlide.height=t;this.currentSlide.setAttribute(b.lazyAttr,
a.slide_image_baseurl+"1"+k);var r=d(this.currentSlide);e.appendChild(this.currentSlide);if(1<this.settings.startAt){var u=c.settings.seqLoad?"data-src":"src";for(g=2;g<=h;g+=1){var f=n.slide.cloneNode(!1);f.setAttribute(u,a.slide_image_baseurl+g+k);f.onload=c.onLoadHandler;e.appendChild(f)}m=!0}var v=n.$next[0].cloneNode(!0);v.title=this.settings.nextTitle;e.appendChild(v);this.counter=n.count.cloneNode(!0);this.counter.firstChild.appendChild(p.createTextNode(h));e.appendChild(this.counter);!1!==
this.settings.linkUrl&&(0===this.settings.linkUrl&&(this.settings.linkUrl=this.sourceUrl+"/",l=!0),0===this.settings.linkIcon&&(this.settings.linkIcon=/^https?:\/\/[^\/]+/.exec(this.settings.linkUrl)+"/favicon.ico"),f=n.$link[0].cloneNode(!0),f.title=this.settings.linkTitle,f.href=this.settings.linkUrl,f.firstChild.src=this.settings.linkIcon,e.appendChild(f));this.elem.insertBefore(e,this.elem.firstChild);this.currentSlide.onload=function(){this.className.match(/\benabled\b/)?(--c.toLoad,c.loading(!1),
1<h&&(v.style.display="block",c.counter.style.display="block",c.counter=c.counter.firstChild.firstChild,d(v).one("click",function(){c.loading(!0);e=p.createDocumentFragment();if(!m)for(u=c.settings.seqLoad?"data-src":"src",g=2;g<=h;g+=1)f=n.slide.cloneNode(!1),f.setAttribute(u,a.slide_image_baseurl+g+k),f.onload=c.onLoadHandler,e.appendChild(f);f.className="lastSlide";f=n.$prev[0].cloneNode(!0);f.title=c.settings.prevTitle;e.appendChild(f);0<c.settings.skipFTitle.length&&(f=n.$skipF[0].cloneNode(!0),
f.title=c.settings.skipFTitle,e.appendChild(f));0<c.settings.skipBTitle.length&&(f=n.$skipB[0].cloneNode(!0),f.title=c.settings.skipBTitle,e.appendChild(f));c.elem.insertBefore(e,this);c.settings.seqLoad&&(r.next()[0].src=r.next()[0].getAttribute("data-src"));c.$elem.delegate(".controlSlide","click",function(){this.className.match(/\bskipFSlide\b/)?c.currentSlide.className.match(/\blastSlide\b/)||c.fadeInSlide(c.elem.children[h-1],l):this.className.match(/\bskipBSlide\b/)?c.currentSlide.className.match(/\bfirstSlide\b/)||
c.fadeInSlide(c.elem.firstChild,l):this.className.match(/\bnextSlide\b/)?c.currentSlide.className.match(/\blastSlide\b/)||c.fadeInSlide(c.currentSlide.nextSibling,l):this.className.match(/\bprevSlide\b/)&&(c.currentSlide.className.match(/\bfirstSlide\b/)||c.fadeInSlide(c.currentSlide.previousSibling,l))})})),c.$elem.delegate(".controlSlide",{mouseenter:function(){d(this).stop(!1).animate({opacity:.99},c.settings.fadeDuration)},mouseleave:function(){d(this).stop(!1).animate({opacity:b.cssOpacity},
